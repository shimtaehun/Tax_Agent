...
- [x] 부가세(4종) · 종합소득세 · 법인세 자동 등록 — `POST /api/v1/deadlines/generate`
- [x] 고객사별 기한 목록 조회 — `GET /api/v1/deadlines`
- [x] 신고 완료 체크 및 이력 저장 — `POST /api/v1/deadlines/{id}/complete`
- [ ] D-7, D-1 이메일/슬랙 알림 (외부 서비스 연동 필요)
...
- [x] 멀티파일 업로드 (최대 20개) — `POST /api/v1/receipts/batch`
- [x] 파일별 독립 처리 — 중복/오류는 skipped/error 처리, 배치 전체를 실패시키지 않음
- [x] `batch_id` 로 배치 추적 (Migration 0004)
- [ ] ZIP 파일 업로드 → 자동 압축 해제
- [ ] 처리 실패 항목 재시도 UI (프론트엔드)
...
- [x] 각 판단에 사용된 법령 조문 직접 링크 — `GET /api/v1/receipts/{id}/explanation`
- [x] 처리 감사 이력 (audit_trail) 반환
- [x] 판단 메타데이터 — confidence, prompt_version, model_name, law_corpus_version
- [ ] 과거 동일 유형 영수증과 자동 비교
